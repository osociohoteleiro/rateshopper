...
import { ITarifa, IApiResponse, IPaginatedResponse, IPaginacao, IFiltroTarifa } from '../types';
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const filtros: IFiltroTarifa = {
      ...
      preco_min: req.query.preco_min ? parseFloat(req.query.preco_min as string) : undefined,
      preco_max: req.query.preco_max ? parseFloat(req.query.preco_max as string) : undefined,
      canal: req.query.canal as string,
      tipo_quarto: req.query.tipo_quarto as string
    };
    ...
    const response: IPaginatedResponse<ITarifa[]> = {
      success: true,
      data: rows,
      pagination
    };
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const response: IApiResponse<ITarifa> = {
      success: true,
      data: tarifa
    };
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const tarifaCompleta = await Tarifa.findByPk(novaTarifa.id, {
      include: [{
        model: Hotel,
        as: 'hotel',
        attributes: ['id', 'nome', 'localizacao']
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const tarifaAtualizada = await Tarifa.findByPk(id, {
      include: [{
        model: Hotel,
        as: 'hotel',
        attributes: ['id', 'nome', 'localizacao']
